...
class LinkedList{
	private:
		...
		Iterator<Object> FindPrevByPosition(int Position) const throw(){
			if(Position < 0 || Position > Count()) throw Tasma();
			int index=0;
			Iterator<Object> iterator(head);
			
			while(!iterator.hasNext() && Position != index++) iterator.next();
			return iterator;
		}		
	...
	public:
		LinkedList(){
			head = new Node<Object>(); // Boþ bir düðüm oluþturuluyor	
			size=0;
		}
		bool isEmpty() const{
